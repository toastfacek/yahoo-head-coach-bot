...
describe('Scout Tool - Data Processing', () => {
  ...
  describe('roster analysis', () => {
    it('identifies roster construction issues', () => {
      ...
      // Injured starters
      const injuredStarters = mockRoster.filter(
        (p) => p.selectedPosition !== 'BN' && /^(O|Q|D|IR)$/i.test(p.status)
      );
      expect(injuredStarters).toHaveLength(2);

      // Available healthy backups
      const healthyBackups = mockRoster.filter(
        (p) => p.selectedPosition === 'BN' && p.status === 'HEALTHY'
      );
  ...
  describe('news signal processing', () => {
    ...
    it('matches news to roster players', () => {
      ...
      const relevantNews = newsSignals.filter((news) => rosterPlayerIds.includes(news.playerId));
