...
export async function hourly(req: Request, res: Response) {
  ...
  try {
    ...
    return res.json({ ran: true, startedAt, hour, zone: 'America/New_York', length: text?.length ?? 0 });
  } catch (error: any) {
    console.error('scheduler/hourly error:', error);
    return res.status(500).json({ ran: false, error: 'agent_error', message: error?.message || 'Unknown error' });
  }
}
