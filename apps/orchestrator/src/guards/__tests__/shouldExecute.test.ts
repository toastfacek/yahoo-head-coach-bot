...
describe('shouldAutoExecute', () => {
  describe('confidence thresholds', () => {
    it('returns false when confidence below execute threshold, even for injury-out swap', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.79, isInjuryOut: true })).toBe(
        false
      );
    });

    it('auto-executes injury-out LINEUP_SWAP when confidence is high', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.85, isInjuryOut: true })).toBe(
        true
      );
    });

    it('respects exact confidence boundary at 80%', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.8, isInjuryOut: true })).toBe(
        true
      );
    ...
    it('handles high confidence values (>1) correctly', () => {
      ...
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 80, isInjuryOut: true })).toBe(
        true
      );
  ...
  describe('WAIVER auto-execution rules', () => {
    ...
    it('does not auto-execute WAIVER when fabBid or fabRemaining missing', () => {
      expect(shouldAutoExecute({ type: 'WAIVER', confidence: 0.9 })).toBe(false);

      expect(shouldAutoExecute({ type: 'WAIVER', confidence: 0.9, fabBid: 5 })).toBe(false);

      expect(shouldAutoExecute({ type: 'WAIVER', confidence: 0.9, fabRemaining: 100 })).toBe(false);
    });
  ...
  describe('LINEUP_SWAP rules', () => {
    it('requires injury-out status for auto-execution', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.9, isInjuryOut: false })).toBe(
        false
      );

      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.9 })).toBe(false);
    });

    it('requires both high confidence AND injury-out status', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.75, isInjuryOut: true })).toBe(
        false
      );

      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 0.85, isInjuryOut: false })).toBe(
        false
      );
    });
  });
  ...
  describe('edge cases and validation', () => {
    it('handles unknown action types safely', () => {
      expect(shouldAutoExecute({ type: 'UNKNOWN_TYPE' as any, confidence: 0.9 })).toBe(false);
    });
    ...
    it('handles extreme confidence values', () => {
      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: 1.5, isInjuryOut: true })).toBe(
        true
      );

      expect(shouldAutoExecute({ type: 'LINEUP_SWAP', confidence: -0.1, isInjuryOut: true })).toBe(
        false
      );
    });
